...
static error_t sendPublish(uint32_t addrHi, uint32_t addrLo)
{
  ...
  buf[5] = RFPT_MSG_DEV_LIGHT;

  return xbee_send(addrHi, addrLo, buf, 6, &id);
}
 ...
 *
 * Description:
 *   Send value read from light sensor
 *
 * Params:
 *   [in] addrHi - upper 32 bits of the 64-bit address
 *   [in] addrLo - lower 32 bits of the 64-bit address
 *
 *****************************************************************************/
static error_t sendLight(uint32_t addrHi, uint32_t addrLo)
{
  uint8_t id = 0;
  uint8_t buf[3];
  uint16_t v = light_read();

  buf[0] = (RFPT_MSG_VAL|RFPT_MSG_DEV_LIGHT);
  buf[1] = ((v >> 8) & 0xff);
  buf[2] = (v & 0xff);
    ...
    uint8_t* buf, int16_t len)
{
  ...
  case RFPT_MSG_DEV_TEMP:
    ...
    // using value from trimpot as temperature
    cval = trimpot_read();
    break;
  case RFPT_MSG_DEV_LIGHT:
    val = (int16_t)(buf[1] << 8 | buf[2]);
    pId = RFPT_MSG_DEV_LIGHT;
    ...
    uint8_t* buf, int16_t len)
{
  ...
  case RFPT_MSG_DEV_RGB:
    ...
    // using LED4 - LED6 connected to PCA9532
    // instead of the RGB LED
  ...
  case RFPT_MSG_DEV_LED:
    ...
    // using LED19 connected to PCA9532
    if (buf[0] == 1) {
      on = LED19;
    }
    else {
        ...
        if (subscribers[i].addrHi != 0 && subscribers[i].addrLo) {
          ...
          if (update) {
            ...
            case RFPT_MSG_DEV_TEMP:
              ...
              break;
            case RFPT_MSG_DEV_LIGHT:
              sendLight(subscribers[i].addrHi, subscribers[i].addrLo);
              break;
            case RFPT_MSG_DEV_BTN:
